...
teamRoute.post('/teams', async (req: Request, res: Response) => {
  ...
  try {
    // Check if the team leader with the provided teamLeaderId exists
  ...
  teamRoute.put('/teams/:id', async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (team) {
        // Check if the team leader with the provided teamLeaderId exists in the Team table
        const teamLeader = await Team.findByPk(teamLeaderId);
        if (!teamLeader) {
